...
class CoverLetterGenAdapter(LetterAdapter):
  ...
  async def generate(
      ...
      *,
      ...
      logger.debug(
          "cover_letter_gen final: provider=%s has_letter=%s effective_passed=%s "
          "violations=%d confidence=%.2f selected_project=%s attempts=%s word_count=%s error=%s",
          self.name,
          bool(letter),
          bool(effective_passed),
          len(effective_violations),
