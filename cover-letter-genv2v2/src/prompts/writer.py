...
_HUMAN_TONE_RULES = """\
ПРОТИВ AI-СТИЛЯ (пиши как живой человек, а не как нейросеть)
- Не начинай предложения подряд одинаково и не строй все факты по одному шаблону. Меняй зачины: «Создал…», «Перевёл…», «Отвечал за…», «Вёл в одиночку…». Слово «единолично» — не более одного раза на письмо.
- ЗАПРЕЩЕНЫ общие маркетинговые эпитеты и AI-штампы: «динамичный», «инновационный», «уникальный», «безупречный», «передовой», «комплексный подход», «играет ключевую роль», «не только… но и», «в современном мире».
- ЗАПРЕЩЕНЫ книжные вводные в начале предложений: «Безусловно», «Несомненно», «Следовательно», «Кроме того», «Более того», «Таким образом», «В заключение».
- Не сваливай термины в одну длинную перечислительную цепочку через запятую (не более 3 подряд). Лучше показать один-два факта и что они дали продукту или пользователю.
- Конкретный факт всегда лучше оценочного эпитета: показывай, что именно сделал и каков результат, а не хвали себя прилагательными.
"""
...
""" + _NO_YEARS_OPENER_RULES + _HUMAN_TONE_RULES + """\
