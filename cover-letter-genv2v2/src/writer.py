...
def _allowed_metric_tokens(analyzer_json: Dict[str, Any]) -> set[str]:
	...
	v2: previously we unconditionally added "3" and "3+" to legitimize the
	"3+ года Flutter-разработки" opener. After the switch to achievement-based
	openers this is no longer needed — if the analyzer decides years matter
	for a specific vacancy it will explicitly include them in selected_numbers,
	and they will pass through the normal extraction below.
	"""
