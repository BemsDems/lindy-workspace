...
1. СТРУКТУРА (НЕ НАРУШАТЬ):
- STANDARD режим: РОВНО 2 абзаца, разделённых ОДНОЙ пустой строкой (\n\n между ними). Не 1, не 3.
- UNIVERSAL режим: 1 плотный абзац.
- Никаких списков, буллитов, заголовков, markdown.

2. ПЕРВЫЙ АБЗАЦ - привязка к вакансии + 1-2 конкретных факта из ACHIEVEMENTS:
- НЕ начинай со слов «X+ лет опыта», «X+ года коммерческой разработки», «Меня заинтересовала ваша вакансия», «Я узнал о вакансии», «Хочу предложить свою кандидатуру». Это маркеры автоматизации.
- ВМЕСТО этого: либо отреагируй на HOOK / роль / домен из VACANCY_CONTEXT, либо начни с конкретного факта-кейса из ACHIEVEMENTS.
- В этом же абзаце - 1-2 числовых факта из SELECTED_NUMBERS.

3. ВТОРОЙ АБЗАЦ (STANDARD) - связка с компанией + живой финал:
- 1 предложение про то, что заинтересовало в продукте / домене / задачах компании (используй VACANCY_CONTEXT).
- Живой финал (см. примеры ниже) - НЕ шаблонный.

4. НИКОГДА не заканчивай шаблонными фразами:
- «Готов обсудить задачи и подробнее рассказать о релевантном опыте на собеседовании.»
- «Буду рад обсудить.»
- «Хотел бы…»
- «Этот опыт поможет…», «Смогу быстро включиться…», «Буду полезен…»

5. Подпись НЕ добавляй - её добавит постпроцесс.
...
Если в названии вакансии есть слова «спикер», «преподаватель», «ментор», «тренер», «evangelist», «advocate»:
- ОБЯЗАТЕЛЬНО упомяни опыт публичных выступлений, менторинга, проведения митапов, code review или обучения команды (если такой опыт есть в ACHIEVEMENTS).
- Если такого опыта в ACHIEVEMENTS нет - всё равно покажи готовность делиться знаниями и сделай акцент на структурном мышлении (Clean Architecture, документирование решений).

Если в названии вакансии есть слова «тимлид», «teamlead», «архитектор», «senior+», «principal»:
- Покажи опыт принятия архитектурных решений, ревью кода, наставничества.
...
def _finalizer_user(
    *,
    ...
    if vacancy_description:
        # Trim description to keep prompt manageable but keep enough to give domain signal.
    ...
    if not universal_mode:
        parts.append("Напиши письмо по MODE: РОВНО 2 абзаца через пустую строку. Первый абзац - привязка к VACANCY_CONTEXT + 1-2 факта из ACHIEVEMENTS с числами. Второй абзац - связка с компанией/доменом из VACANCY_CONTEXT + живой финал. Верни только письмо, без комментариев.")
    else:
