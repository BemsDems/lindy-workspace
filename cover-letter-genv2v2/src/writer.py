...
- Если в названии вакансии или описании есть нестандартная роль (спикер, преподаватель, ментор, тимлид, архитектор) - явно отвечать на эту роль.
...
B. ЗАПРЕТ НА «ГОДЫ ОПЫТА» В ОПЕНЕРЕ. Первое содержательное предложение после GREETING НЕ должно начинаться с конструкции про годы опыта («X+ лет», «X года», «Имею X+ лет опыта», «Опыт работы X+ лет», «Более X лет», «Около X лет»). ВМЕСТО этого: первое предложение = конкретный факт-кейс из ACHIEVEMENTS, либо прямой ответ на HOOK.
...
- «Буду рад обсудить.», «Буду рад обсудить детали.»
- «Хотел бы обсудить...», «Этот опыт поможет...», «Этот опыт может быть полезен...»
- «Смогу быстро вклиниться...», «Буду полезна...»
...
F. ЗАПРЕТ НА ВЫДУМАННЫЕ ФАКТЫ. Эти классы утверждений ЗАПРЕЩЕНЫ в любой части письма, если их НЕТ дословно в ACHIEVEMENTS:
- F.1. Проценты эффективности/ускорения/снижения («улучшил на 30%», «ускорил на 25%», «снизил crash rate на X%», «повысил конверсию на X%»). Любое «X%» рядом со словами «быстрее», «улучшение», «снижение», «рост», «оптимизация».
- F.2. Цифры аудитории («X млн пользователей», «X тыс. установок», «MAU/DAU = X», «X+ компаний используют»).
- F.3. Конкретные времена и длительности созвонов («20-минутный созвон», «после 17:00», «в будни», «в пятницу вечером»). Финал должен быть живым, НО БЕЗ выдумывания таймслотов.
- F.4. Обещания будущего с числами («выйду на full performance за 2 недели», «закрою задачу за X дней»).

G. HOOK ЗАКРЫТ ЯВНО (КРИТИЧНО). Если в блоке HOOK передана непустая фраза, ПЕРВОЕ содержательное предложение после GREETING ОБЯЗАНО буквально отвечать на эту фразу:
- HOOK про преподавание / менторинг / спикерство / обучение → первое предложение содержит слова уровня «преподавал», «менторил», «объяснял», «выступал», «вёл курс», «делился знаниями», «проводил code review для команды», «помогал джунам разбираться» — с конкретным фактом-кейсом из ACHIEVEMENTS.
- HOOK про архитектуру / тимлид / принятие решений → первое предложение содержит конкретное решение (миграция, переписать модуль, ввести процесс, выбрать стек).
- HOOK про конкретный домен (стриминг, финтех, медтех) → первое предложение показывает опыт в этом или соседнем домене из ACHIEVEMENTS.
- HOOK про конкретную технологию из ACHIEVEMENTS → первое предложение её упоминает с фактом-кейсом.
Не вскользь, не во втором абзаце. БУКВАЛЬНО — первое предложение, явные слова из категории HOOK. Если ничего подходящего НЕТ в ACHIEVEMENTS — используй опыт ближайшего класса (структурный подход, разбор кода, документирование) и сделай это первым предложением.

=== СТРУКТУРА ===

STANDARD режим (2 абзаца, 70-110 слов СУММАРНО — это ЖЁСТКИЙ потолок, не превышай):

Абзац 1 (3-4 предложения):
- Если HOOK не пустой — первое предложение БУКВАЛЬНО закрывает HOOK (правило G).
- Иначе — конкретный факт-кейс или прямой ответ на VACANCY_TITLE.
- НЕ начинай с «X+ лет», «Меня заинтересовала ваша вакансия», «Я узнал о вакансии».
- 1-2 числа из SELECTED_NUMBERS.
- Привязка к ALLOWED_TECH (не более 3 технологий подряд).

Абзац 2 (2-3 предложения):
- 1 предложение про то, что заинтересовало в продукте/домене/задачах компании.
- Живой, конкретный финал.

UNIVERSAL режим: 1 плотный абзац 60-90 слов (правило G всё равно действует).

=== ОБРАБОТКА СПЕЦИАЛЬНЫХ РОЛЕЙ ===

Если в VACANCY_TITLE/DESCRIPTION есть «спикер», «преподаватель», «ментор», «тренер», «evangelist», «advocate», «лектор», «курс»:
- Первый абзац — про опыт публичных выступлений, менторинга, проведения митапов, code review или обучения команды.
- Если такого опыта в ACHIEVEMENTS нет — акцент на структурном мышлении, документировании решений, готовности делиться знаниями (КОНКРЕТНО, с примером).

Если в VACANCY_TITLE есть «тимлид», «teamlead», «архитектор»:
...
- Не используй англицизмы-кальки: «пайплайн», «юзер», «коммитить», «пушить», «митап», «фича», «стейкхолдер», «роадмап», «таска», «фиксить», «дебажить». Тех. термины латиницей (Flutter, BLoC, gRPC, API) — норма.
...
def _finalizer_user(
	*,
	...
	mode = (
		...
		if universal_mode
		else "STANDARD: РОВНО 2 абзаца через \\n\\n, 70-110 слов суммарно (ЖЁСТКИЙ потолок 110)"
	)
	...
	parts: List[str] = [
		...
		greeting,
		"",
		"=== HOOK (правило G — ПЕРВОЕ содержательное предложение БУКВАЛЬНО закрывает HOOK) ===",
	...
	parts.append("")
	parts.append("ALLOWED_TECH:")
	for t in allowed_tech[:80]:
		parts.append(f"- {t}")

	parts.append("")
	parts.append("OPENERs (СМЫСЛ одного — первая содержательная фраза ПОСЛЕ GREETING; адаптируй под вакансию, НЕ начинай с «X+ лет»):")
	for o in openers:
		parts.append(f"- {o}")
	...
	if not universal_mode:
		parts.append(
			"Напиши письмо по MODE. ЖЁСТКИЙ потолок 110 слов суммарно — не превышай. "
			"СТРУКТУРА: GREETING слово в слово → пустая строка → абзац 1 (3-4 предложения, ПЕРВОЕ = буквальный ответ на HOOK по правилу G + 1-2 факта из ACHIEVEMENTS с числами) → пустая строка → абзац 2 (2-3 предложения: связка с компанией/доменом + живой финал). "
			"Верни только письмо, без комментариев."
		)
	else:
		parts.append(
			"Напиши письмо по MODE. ЖЁСТКИЙ потолок 90 слов. "
			"СТРУКТУРА: GREETING слово в слово → пустая строка → 1 плотный абзац. ПЕРВОЕ содержательное предложение = буквальный ответ на HOOK (правило G) + конкретный факт-кейс из ACHIEVEMENTS. "
			"Верни только письмо, без комментариев."
		)
	return "\n".join(parts)
...
- Первое предложение письма (после "Здравствуйте...") НЕ должно начинаться с конструкции про годы опыта ("X+ лет", "X лет", "X+ года", "X года", "Имею X+ лет опыта", "Опыт работы X+ лет", "Более X лет", "Около X лет", "Имею X+ лет опыта", "Опыт работы X+ лет", "Хочу предложить свою кандидатуру", "Меня заинтересовала ваша вакансия"). Если в исходном письме такая конструкция уже есть — замени её на конкретный факт-кейс из selected_achievements (что сделал, в каком продукте/домене).
...
- Не присаивай кандидату технологии из вакансии, если их нет в selected_achievements, allowed_tech или evidence.
- Запрещено добавлять как опыта кандидата: video player, DRM, ExoPlayer, HLS, DASH, offline cache, WebSocket, Firestore, Amplitude, AppsFlyer, FFI, Kotlin, Swift, platform channels, MethodChannel, EventChannel.
